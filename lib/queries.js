...
class Queries { 
  ...
  updateEmployeeRole() { 
    ...
    const { connection, loadMainPrompts } = this;
    const { employee, role } = this.answers;
    const data = { role_id: role };
    const query = `UPDATE employee SET ? WHERE ?`;

    connection.promise().query(query, [data, { id: employee }])
      .then(([rows, fields]) => console.log(`Updated employee ${this.answers.employee}'s role.`))
      .catch(console.error)
      .then(() => loadMainPrompts());
