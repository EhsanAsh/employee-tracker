...
class Queries { 
  ...
  updateEmployeeRole() { 
    ...
    const { connection, loadMainPrompts } = this;
    const { employeeUpdate, employeeNewRole } = this.answers;
    const data = { role_id: employeeNewRole };
    const query = `UPDATE employee SET ? WHERE ?`;

    connection.promise().query(query, [data, { id: employeeUpdate }])
      .then(([rows, fields]) => console.log(`Updated employee ${this.answers.employeeUpdate} role.`))
      .catch(console.error)
      .then(() => loadMainPrompts());
